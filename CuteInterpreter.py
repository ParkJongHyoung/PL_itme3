...
CUTETYPE_NAMES = dict((eval(attr, globals(), CuteType.__dict__), attr) for attr in dir(CuteType())
...
NODETYPE_NAMES = dict((eval(attr, globals(), TokenType.__dict__), attr) for attr in dir(TokenType())
                      if not callable(attr) and not attr.startswith("__"))
...
class BasicPaser(object):
    ...
    def _create_node(self, token):
        ...
        elif is_type_binaryOp(token) or is_type_keyword(token) or is_type_boolean(token):
...
class CuteInterpreter(object):
    ...
    def run_arith(self, arith_node):
        ...
        if rhs1 is None or rhs2 is None:
            print "run_arith run_expr null"

        return {
            TokenType.PLUS: Node(TokenType.INT, int(expr_rhs1.value) + int(expr_rhs2.value)),
            TokenType.MINUS: Node(TokenType.INT, int(expr_rhs1.value) - int(expr_rhs2.value)),
            TokenType.TIMES: Node(TokenType.INT, int(expr_rhs1.value) * int(expr_rhs2.value)),
            TokenType.DIV: Node(TokenType.INT, int(expr_rhs1.value) / int(expr_rhs2.value)),
            TokenType.LT: self.TRUE_NODE if int(expr_rhs1.value) < int(expr_rhs2.value) else self.FALSE_NODE,
            TokenType.GT: self.TRUE_NODE if int(expr_rhs1.value) > int(expr_rhs2.value) else self.FALSE_NODE
        }[arith_node.type]
    ...
    def run_cond(node):
            ...
            elif node.value.type is TokenType.ID:
                cond = lookup_table(node.value.value)
            else:
                cond = node.value
    ...
    def run_lambda(self, func_node):
        ...
        if rhs1 is None :
            ...
            insert_table(param.value, rhs1)
            result = self.run_expr(lambda_func.next.next)
    ...
    def run_func(self, func_node):
        ...
        if func_node.type is TokenType.CAR:
            ...
            if result.type is TokenType.LIST:
                result = expr_rhs1.value.get_tail().next
            ...
            elif rhs1.type is TokenType.ID : # 문자일때
                if lookup_table(rhs1.value).type is TokenType.LIST: # dic에서 찾아온다 & 그 값이 리스트이면
                    return self.FALSE_NODE
                else:
                    return self.TRUE_NODE
            elif rhs1.type is TokenType.LIST: # 리스트일 때
                if rhs1.value.type is TokenType.QUOTE: # QUOTE로 시작하는 상수 문자열
                        if rhs1.value.next.type is TokenType.LIST:
                            return self.FALSE_NODE
                        else:#' 뒤의 값이 List 형태가 아닐 때
                             return self.TRUE_NODE
                else:
                    if self.run_expr(rhs1).type is TokenType.LIST or TokenType.QUOTE:
                        return self.FALSE_NODE
                    else:
                        return self.TRUE_NODE
            else :
            ...
            expr_rhs1 = self.run_expr(rhs1)
            expr_rhs2 = self.run_expr(rhs2)
            if expr_rhs1.value is expr_rhs2.value:
                return self.TRUE_NODE
            return self.FALSE_NODE
    ...
    def run_expr(self, root_node):
        ...
        if root_node.type is TokenType.ID:
            if root_node.type is TokenType.ID:
                value = lookup_table(root_node)
                if value.type is TokenType.LIST:
                    if value.value.type is TokenType.LAMBDA :
                        value.next = self.run_expr(root_node.next)
            return self.run_expr(value)
    ...
    def run_list(self, l_node):
        ...
        if op_code.type in \
                [TokenType.CAR, TokenType.CDR, TokenType.CONS, TokenType.ATOM_Q,TokenType.EQ_Q,\
                 TokenType.NULL_Q, TokenType.NOT, TokenType.DEFINE]:
        ...
        if op_code.type is TokenType.ID:
            return self.run_expr(op_code)

        #if op_code.value in table:

           # print lookup_table(op_code.value).value.next.value.next.value.value
           # insert_table(lookup_table(op_code.value).value.next.value.next.value.value, op_code.next.value)
           # print lookup_table(op_code.value).value.next.value.next.value.value
...
def Test_method():
    ...
    test_basic_parser = BasicPaser(test_tokens)
    node = test_basic_parser.parse_expr()
    cute_inter = CuteInterpreter()
...
def Test_All():
    print "Input 'exit' to terminate program"
    Test_method()
